...
serve(async (req) => {
  ...
  if (!caller || caller.role === 'membre' || caller.role === 'admin')
  ...
  if (!target || target.role === 'chef')
    return new Response(JSON.stringify({ error: 'Impossible d\'exclure ce membre' }), { status: 403, headers: cors })
  await sb.from('clan_members').delete().eq('clan_id', clan_id).eq('user_id', target_user_id)
