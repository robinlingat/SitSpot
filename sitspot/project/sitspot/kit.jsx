/* SitSpot — design primitives */

const Icon = ({ n, s=20, color }) => (
  <i data-lucide={n} style={{ width:s, height:s, color:color||'currentColor', flex:'none', display:'block' }}/>
);

function Stars({ value=0, size=15, interactive, onRate, hover=0 }) {
  return (
    <span style={{ display:'inline-flex', gap:2 }}>
      {[1,2,3,4,5].map(i => {
        const disp = hover||value;
        const fill = disp>=i ? 'var(--star)' : disp>=i-0.5 ? 'url(#sg)' : 'var(--neutral-200)';
        return (
...
function Btn({ children, variant='primary', size='md', full, onClick, iconLeft, disabled, style:xStyle }) {
...
function Tag({ children, icon, selected, onClick }) {
  return (
    <button onClick={onClick} style={{
...
function Badge({ children, tone='green', solid, dot }) {
  const p = {
...
function Avatar({ name='', size=40 }) {
  ...
  return (
    <span style={{ display:'grid', placeItems:'center', width:size, height:size,
      borderRadius:'50%', background:tints[idx], color:txts[idx],
      fontWeight:700, fontSize:size*0.38, flex:'none' }}>
      {initials||'?'}
    </span>
  );
}

function Marker({ bench, open, onClick }) {
  const sc = bench.score;
  const dotColor = sc===null ? 'var(--neutral-400)'
    : sc>=4 ? 'var(--green-500)'
    : sc>=2.5 ? 'var(--warning)'
    : 'var(--danger)';
...
function FloatBtn({ icon, label, accent, onClick }) {
  return (
...
Object.assign(window, { Icon, Stars, Btn, Chip, Tag, Badge, Avatar, Marker, FloatBtn, TabBar });
