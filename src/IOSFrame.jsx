  ...
  return (
    <div style={{
      ...
      <div style={{position:'absolute',top:0,left:0,right:0,zIndex:10}}>
        <div style={{display:'flex',gap:154,alignItems:'center',justifyContent:'center',
          padding:'21px 24px 19px',position:'relative',zIndex:20,width:'100%'}}>
...
export function DeviceScaler({ children }) {
  const ref = React.useRef(null);
  React.useEffect(() => {
    const update = () => {
      if (!ref.current) return;
      const pad = 32;
      const s = Math.min((window.innerWidth-pad*2)/402,(window.innerHeight-pad*2)/874,1);
